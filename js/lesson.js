...
if (!moduleId || !lessonId) {
  document.getElementById('lessonContent').innerHTML = '<div style="padding:40px;color:#ff4d4f;">Invalid lesson.</div>';
...
function loadLesson() {
  ...
  if (currentLesson.pdfUrl) {
    html += `<div class="pdf-download"><i class="fa-solid fa-file-pdf"></i> <a href="${currentLesson.pdfUrl}" target="_blank">Download PDF</a></div>`;
  }
  if (currentLesson.notes) {
    html += `<div class="notes-box">${currentLesson.notes}</div>`;
  }
  ...
  html += `
    <div class="actions">
      <button class="btn-primary" id="markCompleteBtn" ${isCompleted ? 'disabled' : ''}>
        <i class="fa-${isCompleted ? 'solid fa-check-circle' : 'solid fa-check'}"></i> ${isCompleted ? 'Completed' : 'Mark Complete'}
      </button>
      <button class="btn-secondary" onclick="window.location.href='premium-academy.html'">Back to Academy</button>
    </div>
    <div class="nav-buttons">
      ...
      ${nextLesson ? `<a href="lesson.html?module=${moduleId}&lesson=${nextLesson.id}" class="btn-secondary">Next <i class="fa-solid fa-arrow-right"></i></a>` : (isLastLesson && quizLink ? `<a href="${quizLink}" class="btn-secondary">Take Quiz <i class="fa-solid fa-arrow-right"></i></a>` : '')}
    </div>
  `;
...
async function markLessonComplete() {
  ...
  try {
    ...
    loadLesson(); // refresh to update button state and progress
  } catch (e) {
    console.error(e);
