...
function calculateStatistics(trades) {
  ...
  if (total === 0) {
    setText('totalTrades', 0);
    setText('winRate', '0.0%');
    setText('profit', '$0.00');
    setText('rrAverage', '0.00');
    setText('currentStreak', 0);
    setText('profitFactor', '0.00');
    setText('bestTrade', '$0.00');
    setText('worstTrade', '$0.00');
    setText('avgWin', '$0.00');
    setText('avgLoss', '$0.00');
    setText('largestRR', '0R');
    setText('expectancy', '0.00');
    return;
  }
  ...
  if (profitEl) {
    profitEl.style.color = netProfit >= 0 ? '#16c784' : '#ea3943';
  }
}
...
function loadDashboardData() {
  ...
  setText('journalLosses', losses);

  // Average RR from all trades
  let totalRR = 0;
  trades.forEach(t => { totalRR += parseFloat(t.rr) || 0; });
  const avgRR = total > 0 ? totalRR / total : 0;
  setText('journalRR', avgRR.toFixed(2) + 'R');

  // Academy progress (from localStorage)
  ...
  if (reviews.length > 0) {
    ...
    setText('mistakesFixed', fixed);
  } else {
    setText('averageScore', '0%');
    setText('bestScore', '0%');
    setText('mistakesFixed', '0');
  }

  // Psychology (mock values)
  const discipline = 82;
  const consistency = 74;
  const riskPsych = 90;
  setText('disciplineScore', discipline + '%');
  if (disciplineBar) disciplineBar.style.width = discipline + '%';
  setText('consistencyScore', consistency + '%');
  if (consistencyBar) consistencyBar.style.width = consistency + '%';
  setText('riskScore', riskPsych + '%');
  if (riskBar) riskBar.style.width = riskPsych + '%';

  // Achievements
  const achievementEls = document.querySelectorAll('.achievement');
  if (achievementEls.length > 0) {
    if (total > 0) achievementEls[0]?.classList.add('unlocked');
    if (total >= 100) achievementEls[2]?.classList.add('unlocked');
  }
}
...
function populateProfile(user) {
  ...
  setText('accStatus', active ? 'Active' : 'Suspended');
  setText('accUid', currentUser?.uid || '—');

  // Join date
  if (user.createdAt) {
    setText('accJoined', formatDate(user.createdAt));
  }
  ...
  if (membershipBadge) {
    ...
    membershipBadge.className = 'badge ' + membership;
  }
  if (statusBadge) {
    statusBadge.textContent = active ? 'ACTIVE' : 'SUSPENDED';
    statusBadge.className = 'badge ' + (active ? 'active' : 'suspended');
  }
...
document.getElementById('downloadDataBtn')?.addEventListener('click', () => {
  const data = {
    ...
    trades: loadTrades(),
    goals: {
      daily: dailyGoal?.value || 0,
      weekly: weeklyGoal?.value || 0,
      monthly: monthlyGoal?.value || 0,
      risk: riskTrade?.value || 0,
      maxLoss: maxLoss?.value || 0,
    }
  };
