...
if (!moduleId) {
  document.getElementById("quizContainer").innerHTML = `
    <div style="padding:40px;color:#ff4d4f;text-align:center;">No module specified.</div>
  `;
...
async function loadQuiz() {
  try {
    ...
    if (!moduleData.hasQuiz || !moduleData.quiz || !moduleData.quiz.questions) {
      container.innerHTML = `<div style="padding:40px;color:#94a3b8;text-align:center;">No quiz for this module.</div>`;
      return;
    }
    ...
    if (modProgress.quizPassed) {
      container.innerHTML = `
        <div class="result-box">
          ...
          <button onclick="window.location.href='academy.html'" class="btn btn-primary">Back to Academy</button>
        </div>
      `;
      return;
    }
...
function renderQuiz() {
  ...
  html += `
    <button type="submit" class="btn btn-primary" style="width:100%;padding:12px;font-size:1rem;">Submit Quiz</button>
  </form>`;
...
async function submitQuiz() {
  ...
  container.innerHTML = `
    <div class="result-box">
      ...
      <div class="${passed ? 'pass' : 'fail'}" style="font-weight:600;font-size:1.1rem;">
        ${passed ? '✅ Passed!' : '❌ Not passed (needs ' + passingScore + '%)'}
      </div>
