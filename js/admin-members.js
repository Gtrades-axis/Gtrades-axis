...
demoteBtn?.addEventListener("click", async () => {
  ...
  if (!confirm(`Demote ${selectedUser.name} back to regular member?`)) return;

  try {
    await updateDoc(doc(db, "users", selectedUser.id), {
      role: "member"
      // keep membership and active status
    });
    ...
    alert("❌ Failed to demote: " + error.message);
  }
});

// In openMember(), show/hide the demote button:
if (selectedUser.role === "admin") {
